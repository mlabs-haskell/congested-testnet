...
RESULT=$(echo "$OGMIOS_REQUEST" | tr -d "\n" | websocat ws://congested-testnet.staging.mlabs.city:1337 )
...
@test 'request kupo' {
curl http://congested-testnet.staging.mlabs.city:1442/matches
}

@test 'request faucet' {
curl http://congested-testnet.staging.mlabs.city:8000
}

@test 'check spammer works based on mempool usage in prometheus statistics' {
RESULT=$(curl 'http://congested-testnet.staging.mlabs.city:9090/api/v1/query?query=round%28avg_over_time%28cardano_node_metrics_mempoolBytes_int%5B1h%5D%29%29')
