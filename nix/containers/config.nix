{ inputs, self, ... }:
{
  perSystem = { system, inputs', pkgs, self', ... }:
    {
      packages.gen-testnet-conf-1 = pkgs.writeShellApplication {
      ...
      packages.gen-testnet-conf = pkgs.writeShellApplication {
        ...
        text = ''
          ...
          cardano-cli conway genesis create-testnet-data \
                  ...
                  --out-dir "$ROOT" 
                  # --relays "$ROOT/topology-spo-1.json" \
          ...
          jq --argjson maxSupply "$MAX_SUPPLY" --argjson secParam "$SECURITY_PARAM" '.maxLovelaceSupply = $maxSupply | .slotLength = 1 | .securityParam = $secParam | .activeSlotsCoeff = 0.05 | .securityParam = $secParam | .epochLength = 432000 | .updateQuorum =2 | .protocolParams.protocolVersion.major = 9 | .protocolParams.minFeeA = 44 | .protocolParams.minFeeB = 155381 | .protocolParams.minUTxOValue = 1000000 | .protocolParams.decentralisationParam = 0.7 | .protocolParams.rho = 0.003 | .protocolParams.tau = 0.2' "$ROOT/shelley-genesis.json" > "$ROOT/temp.json" && mv "$ROOT/temp.json" "$ROOT/shelley-genesis.json"
          touch "$ROOT/finish_config" 
        '';
      };
};
